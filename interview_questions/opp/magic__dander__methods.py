...
__bool__(self)	Вызывается при проверке истинности. Типа: if obj

__call__(self, [args...])	Создает из экземпляра callable-объект, как функцию, например. Должен принимать аргументы.


Контекстные менеджеры
__enter__(self)	То, что должен сделать контекстный менеджер with в начале. Возвращаемое значение __enter__ привязывается
