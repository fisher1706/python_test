...
Разграничительной чертой между наследованием и композицией является тот факт, что при создании обьекта
класса-агрегатора также создаются обьекты агрегируемых классов (надеюсь понятно пишу). В моём понимании,
создание обьекта это код формата "переменная = класс(параметры)",
...
"""
Видно, что при создании обьекта duck класса Duck, в параметры bill и tail конструктора передаются одноимённые обьекты, 
...
Однако метод about обращается не к этим новосозданным атрибутам, а к обьектам агрегируемых классов bill и tail на 
